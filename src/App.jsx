...
function App() {
  ...
  function ChatbotResponse({ content, messageId }) {
    ...
    return <p className="text-sm">{messageId === typingMessageId ? displayedText : content}</p>;
  }
  useEffect(() => {
    getHistory();
  }, []);
  ...
  const getHistory = async () => {
    try {
      const response = await axios.get(`https://financialbot-backend.up.railway.app/history/${userId}`);
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await axios.post('https://financialbot-backend.up.railway.app/chat', {
        message: input,
        userId: userId
      });

      const assistantMessage = { role: 'assistant', content: response.data.response, id: Date.now() };
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      const response = await axios.post('https://financialbot-backend.up.railway.app/upload', formData, {
        headers: {
      ...
      <div className="relative z-10 w-full max-w-2xl bg-white bg-opacity-90 shadow-xl rounded-lg">
        <div className="flex flex-col h-[80vh]">
          <div className="bg-indigo-600 p-4 rounded-t-lg">
            <h1 className="text-2xl font-bold text-white text-center">Chat with Financial AI</h1>
          </div>
          <div className="flex-1 overflow-y-auto p-4" ref={chatContainerRef}>
            {messages.map((message) => (
              <div key={message.id} className={`mb-4 ${message.role === 'user' ? 'text-right' : 'text-left'}`}>
                <div className={`inline-block p-2 rounded-lg ${message.role === 'user' ? 'bg-indigo-500' : 'bg-gray-300 text-black'}`}>
                  {message.role === 'user' ? (
                ...
                <p className="text-xs text-gray-500 mt-1">{message.role === 'user' ? 'You' : 'AI'}</p>
              </div>
            ))}
          </div>
          <div className="border-t p-4">
            ...
            <div className="mt-2 flex items-center">
              ...
              {uploadStatus && <p className="ml-2 text-sm text-black">{uploadStatus}</p>}
            </div>
          </div>
        </div>
      </div>
